import { ServiceData, formatPrice } from './modal-ui';

export default (service: ServiceData | string = {} as ServiceData): string => {
  ...
  try {
    ...
    if (typeof service === 'object' && service !== null &&
      'getConfig' in service && typeof (service as any).getConfig === 'function' &&
      'Panels' in service && typeof (service as any).Panels === 'object') {
    ...
    if (!serviceData.id) {
      ...
      const requiredFields = ['id', 'stripe_product_id', 'name', 'price'];
      const receivedFields = Object.keys(serviceData);
      ...
      return `
        <div class="text-red-600 p-4">
          ...
          <span>Required fields: ${requiredFields.join(', ')}</span><br>
          <span>Received fields: ${receivedFields.join(', ')}</span>
        </div>
      `;
    }

    // Use the bin icon or a placeholder
    let imageUrl = 'https://spotlessbinco.com/assets/bin-icon.png';
    ...
    const buttonText = serviceData.price > 0 ? `Pay ${formattedPrice} & Schedule` : 'Get Started';
            ...
            <ul role="list" class="divide-y divide-white divide-opacity-10 text-sm font-medium mt-6">
              <li class="flex items-start space-x-4 py-6">
                ...
                <div class="flex-auto space-y-1">
                  <h3 class="text-white">${serviceData.name}</h3>
                  <p>${serviceData.description || 'One-time Residential Bin Cleaning'}</p>
                </div>
