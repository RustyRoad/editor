...
export default (editor, opts = {}) => {
  ...
  domc.addType('pricing-card', {
    ...
    model: {
      defaults: {
        tagName: 'div',
        attributes: { 'data-gjs-type': 'pricing-card', class: 'pricing-table-card bg-white dark:bg-gray-900 hover:bg-gray-50 dark:bg-gray-900 dark:bg-gray-800 dark:hover:bg-gray-700 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 p-6 flex flex-col basis-1/3 max-w-1/3 min-w-[250px] flex-1' },
    ...
    model: {
      ...
      renderContent() {
        ...
        if (fetchError) {
          ...
          displayedServices = services.filter(service => selectedProductIds.includes(service.id?.toString()));
          ...
          gridContainer = this.components().filter(comp => comp.get('attributes').class?.includes('pricing-table-grid'))[0];
        ...
        if (displayedServices && displayedServices.length && gridContainer) {
