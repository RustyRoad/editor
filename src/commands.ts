...
interface Options {
	...
	// other optional opts can be added as needed
}
...
type CommandRun = (editor?: Editor, sender?: any, options?: any) => void | Promise<void>;
...
interface CommandsManager {
	add(name: string, cmd: CommandDef): void;
}

interface ComponentsCollection {
	each(cb: (mod: ComponentModel) => void): void;
}

interface ComponentModel {
	components(): ComponentsCollection;
	getAttributes(): { class?: string };
	setClass(cl: string): void;
}

interface DomComponents {
	getWrapper(): ComponentModel;
}
...
const colorRegex: RegExp = new RegExp(
...
const getUpdateThemeModal = (editor: Editor): HTMLElement => {
...
const getAllComponents = (model: ComponentModel, result: ComponentModel[] = []): ComponentModel[] => {
	result.push(model)
	model.components().each((mod) => getAllComponents(mod, result))
	return result
}

const updateThemeColor = (editor: Editor, color: string): void => {
...
export function registerEditorCommands(editor: Editor, opts: Options = {}) {
	const cm = editor.Commands;

	cm.add('open-update-theme', {
		run(editorParam?: Editor, sender?: any) {
			// prefer the passed editor if provided, otherwise use the outer-scoped editor
			const ed = editorParam || editor;
			sender?.set && sender.set('active', 0);
			const md = ed.Modal;
			md.setTitle(opts.changeThemeText || 'Change theme');
			const container = getUpdateThemeModal(ed);
	...
	cm.add('get-tailwindCss', {
		run(editorParam?: Editor, sender?: any, options: { callback?: (twcss: string) => void } = {}) {
			const ed = editorParam || editor;
			try {
				sender?.set && sender.set('active', 0);
			} catch (e) {
